...
function extractTxMetadata(txObj) {
  try {
    ...
    const creationTimeSec = cmdObj.meta?.creationTime || 0;
    const creationTime = new Date(creationTimeSec * 1000).toISOString();
    const gasLimit = cmdObj.meta?.gasLimit || 0;
    const gasPrice = cmdObj.meta?.gasPrice || 0;
    ...
    else if (code.includes('free.radio02.update-sent')) eventType = 'radio02.update';
    else if (code.includes('free.mesh03')) eventType = 'mesh03';

    return { sender, creationTime, eventType, gas };
  } catch {
    return { sender: 'unknown', creationTime: 'invalid', eventType: 'unknown', gas: 0 };
  }
}
