...
async function handleEvent(jsonStr) {
  try {
    ...
    for (const encoded of payload.transactions) {
      let outer, tx;
      try {
        outer = JSON.parse(Buffer.from(encoded, 'base64').toString('utf8'));
        tx = JSON.parse(outer.cmd); // the actual transaction
      } catch (err) {
        console.warn('⚠️ Could not decode transaction:', err.message);
        continue;
      }
      ...
      const code = tx.payload?.exec?.code || '';
      ...
      const enrichedTx = {
        ...
        eventType,
        gas,
        tx: outer, // includes hash, sigs, cmd
      };
