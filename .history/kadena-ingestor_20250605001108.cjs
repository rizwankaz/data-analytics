...
const getBlockHeader = async (chainId, height, retries = 5) => {
  ...
  sse.onmessage = async (msg) => {
    try {
      ...
      if (!payloadHash) {
        ...
        try {
          payloadHash = await getBlockHeader(chainId, height);
        } catch (err) {
          console.error(`❌ Failed to process block ${height} on chain ${chainId}: ${err.message}`);
          return;
        }
      }
